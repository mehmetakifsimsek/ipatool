...
func initWithCommand(cmd *cobra.Command) {
	...
	// Remove a stale zero-byte cookie lock file on startup
	lockFile := filepath.Join(dependencies.Machine.HomeDirectory(), ConfigDirectoryName, CookieJarFileName+".lock")
	if info, err := os.Stat(lockFile); err == nil && info.Size() == 0 {
